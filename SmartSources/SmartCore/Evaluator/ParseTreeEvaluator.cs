...
namespace Smart.Evaluator
{
    ...
    [Serializable]
    public class ParseTreeEvaluator : ParseTree
    {
        ...
        {
            return GetValue(tree, TokenType.Expression, 0);
        }

        protected override object EvalUnaryExpression(ParseTree tree, params object[] paramlist)
        {
            var type = nodes[0].Token.Type;
            if (type == TokenType.PrimaryExpression)
                return GetValue(tree, TokenType.PrimaryExpression, 0);

            if (type == TokenType.MINUS)
            {
                var val = GetValue(tree, TokenType.UnaryExpression, 0);
                if (val is double)
                    return -(double)val;
        ...
        protected override object EvalParams(ParseTree tree, params object[] paramlist)
        {
            ...
            for (var i = 0; i < nodes.Count; i += 2)
            {
                ...
                {
                    var val = nodes[i].Eval(tree, paramlist);
                    parameters.Add(val);
                }
            }
            return parameters;
        ...
        protected override object EvalFunction(ParseTree tree, params object[] paramlist)
        {
            ...
            var paramNode = nodes[2];

            var root = tree as ParseTreeEvaluator;
            if (root == null)
            {
            ...
            var key = funcNode.Token.Text.ToLowerInvariant();
            ...
            var func = root.Context.Functions[key];
            ...
            {
                tree.Errors.Add(new ParseError("At least " + func.MinParameters + " parameter(s) expected", 1043, this));
                return null; // illegal number of parameters
            }
            if (parameters.Length > func.MaxParameters)
            {
                tree.Errors.Add(new ParseError("No more than " + func.MaxParameters + " parameter(s) expected", 1044, this));
                return null; // illegal number of parameters
            }
        ...
        protected override object EvalPrimaryExpression(ParseTree tree, params object[] paramlist)
        {
            ...
            switch (type)
            {
                case TokenType.Function: return GetValue(tree, TokenType.Function, 0);
                case TokenType.Literal: return GetValue(tree, TokenType.Literal, 0);
                case TokenType.ParenthesizedExpression: return GetValue(tree, TokenType.ParenthesizedExpression, 0);
                case TokenType.Variable: return GetValue(tree, TokenType.Variable, 0);
            }
        ...
        {
            var result = GetValue(tree, TokenType.UnaryExpression, 0);
            ...
            for (var i = 1; i < nodes.Count; i += 2)
            {
                ...
                var val = nodes[i + 1].Eval(tree, paramlist);
        ...
        {
            var result = GetValue(tree, TokenType.PowerExpression, 0);
            for (var i = 1; i < nodes.Count; i+=2 )
            {
                var token = nodes[i].Token;
                var val = nodes[i+1].Eval(tree, paramlist);
        ...
        {
            var result = GetValue(tree, TokenType.MultiplicativeExpression, 0);
            for (var i = 1; i < nodes.Count; i += 2)
            {
                var token = nodes[i].Token;
                var val = nodes[i + 1].Eval(tree, paramlist);
        ...
        {
            var result = GetValue(tree, TokenType.AdditiveExpression, 0);
            for (var i = 1; i < nodes.Count; i += 2)
            {
                var token = nodes[i].Token;
                var val = nodes[i + 1].Eval(tree, paramlist);
        ...
        {
            var result = GetValue(tree, TokenType.ConcatEpression, 0);
            for (var i = 1; i < nodes.Count; i += 2)
            {
                var token = nodes[i].Token;
                var val = nodes[i + 1].Eval(tree, paramlist);
                ...
                {
                    var comp = string.Compare(Convert.ToString(result), Convert.ToString(val));
                    if (token.Type == TokenType.LESSTHAN)
                        result = comp < 0;
                    else if (token.Type == TokenType.LESSEQUAL)
                        result = comp <= 0;
        ...
        {
            var result = GetValue(tree, TokenType.RelationalExpression, 0);
            for (var i = 1; i < nodes.Count; i += 2)
            {
                var token = nodes[i].Token;
                var val = nodes[i + 1].Eval(tree, paramlist);
                if (token.Type == TokenType.EQUAL)
                    result = Equals(result, val);
        ...
        {
            var result = GetValue(tree, TokenType.EqualityExpression, 0);
            for (var i = 1; i < nodes.Count; i += 2)
            {
                var token = nodes[i].Token;
                var val = nodes[i + 1].Eval(tree, paramlist);
        ...
        {
            var result = GetValue(tree, TokenType.ConditionalAndExpression, 0);
            for (var i = 1; i < nodes.Count; i += 2)
            {
                var token = nodes[i].Token;
                var val = nodes[i + 1].Eval(tree, paramlist);
        ...
        {
            var result = GetValue(tree, TokenType.ConditionalOrExpression, 0);
        ...
        protected override object EvalExpression(ParseTree tree, params object[] paramlist)
        {
            ...
            if (nodes.Count == 1)                 
                return GetValue(tree, TokenType.AssignmentExpression, 0); // return the result
            ...
            // ok, this is an assignment so declare the function or variable
                ...
                var v = GetFunctionOrVariable(nodes[0]);
                if (v == null)
                {
                ...
                if (v.Token.Type == TokenType.VARIABLE)
                {
                    ...
                    var key = v.Token.Text;
                    root.Context.Globals[key] = GetValue(tree, TokenType.AssignmentExpression, 1);
                    return null;
                }
            if (v.Token.Type == TokenType.Function)
            {

                var key = v.Nodes[0].Token.Text;
                ...
                {   // function has parameters, so check if they are all variable declarations
                    ...
                    {
                        var varNode = GetFunctionOrVariable(paramsNode.Nodes[i]);
                ...
                var dynf = new DynamicFunction(key, nodes[2], vars, vars.Count, vars.Count);
        ...
        protected override object EvalLiteral(ParseTree tree, params object[] paramlist)
        {
            ...
            switch (type)
            {
                case TokenType.StringLiteral: return GetValue(tree, TokenType.StringLiteral, 0);
                case TokenType.IntegerLiteral: return GetValue(tree, TokenType.IntegerLiteral, 0);
                case TokenType.RealLiteral: return GetValue(tree, TokenType.RealLiteral, 0);
                case TokenType.BOOLEANLITERAL:
                    var val = GetValue(tree, TokenType.BOOLEANLITERAL, 0).ToString();
        ...
        {
            if (GetValue(tree, TokenType.DECIMALINTEGERLITERAL, 0) != null)
                return Convert.ToDouble(GetValue(tree, TokenType.DECIMALINTEGERLITERAL, 0));

            if (GetValue(tree, TokenType.HEXINTEGERLITERAL, 0) != null)
            {
                var hex = GetValue(tree, TokenType.HEXINTEGERLITERAL, 0).ToString();
        ...
        {
            if (GetValue(tree, TokenType.REALLITERAL, 0) != null)
            {
                var val = string.Format(CultureInfo.InvariantCulture, "{0}", GetValue(tree, TokenType.REALLITERAL, 0));
        ...
        {
            if (GetValue(tree, TokenType.STRINGLITERAL, 0) != null)
            {
                var r = (string)GetValue(tree, TokenType.STRINGLITERAL, 0);
